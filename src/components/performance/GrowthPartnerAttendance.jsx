...
export default function GrowthPartnerAttendance({ name, roleLabel = 'Growth Partner' }) {
  ...
  const clockOutAvailableAt = record?.clockOutAvailableAt ? new Date(record.clockOutAvailableAt) : record?.clockIn ? new Date(new Date(record.clockIn).getTime() + 5 * 60 * 60 * 1000) : null;
